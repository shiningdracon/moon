...
function censor($message) {
	...
	if($_DCACHE['censor']['banned']) {
		$bbcodes = 'b|i|u|color|size|font|align|list|indent|url|email|hide|quote|code|free|table|tr|td|img|swf|attach|payto|float|dice'.($_DCACHE['bbcodes_display'] ? '|'.implode('|', array_keys($_DCACHE['bbcodes_display'])) : '');
...
function discuzcode($message, $smileyoff, $bbcodeoff, $htmlon = 0, $allowsmilies = 1, $allowbbcode = 1, $allowimgcode = 1, $allowhtml = 0, $jammer = 0, $parsetype = '0', $authorid = '0', $allowmediacode = '0', $pid = 0) {
	...
	if(!$bbcodeoff && $allowbbcode) {
		...
		if(strpos($msglower, '[/dice]') !== FALSE) {
			srand($pid+364);
			$message = preg_replace("/\[dice\](\d+?)[dD](\d+?)([\+\-]\d+?)?\[\/dice\]/ies", "parsedice('\\1', '\\2', '\\3', ".$pid.")", $message);
		}
...
function parsedice($number, $side, $modifier, $seed) {
	$int_number = (int)$number;
	$int_side = (int)$side;
	$int_modifier = (int)$modifier;

	if ($int_number > 20)
		return '<div class="blockcode"><b>Roll&nbsp;&nbsp;&nbsp;</b>('.($int_number.'d'.$int_side.')'.($int_modifier >= 0 ? '+'.$int_modifier : $int_modifier)).' = '.'Too many dices. &lt; 20 please.</div>';

	$random = 0;
	$result_string = '';
	for ($i=0; $i<$int_number; $i++) {
		$temp = rand(1, $int_side);
		$random += $temp;
		$result_string = $result_string.$temp.',';
	}
	return '<div class="blockcode"><b>Roll&nbsp;&nbsp;&nbsp;</b>('.($int_number.'d'.$int_side.')'.($int_modifier >= 0 ? '+'.$int_modifier : $int_modifier)).' = '.$result_string.'&nbsp;'.($int_modifier >= 0 ? '+'.$int_modifier : $int_modifier).' = '.($random + $int_modifier).'</div>';
}
